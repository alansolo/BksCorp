...
//VARIABLES GLOBALES
///////////////////
$tituloCorreoThird = "Actualizar - Carga masiva - Organizaciones";
$tituloCorreoContact = "Actualizar - Carga masiva - Contactos";
...
$minEjecucionProgramada = 3; //Minutos Integer
$limiteElementosApi = "300"; //Cantidad registros a actualizar


date_default_timezone_set('America/Mexico_City');
//$dateSisIni = str_replace(' ', '%20', date("Y-m-d HHmmss", strtotime('-10 minutes')));
$dateSis = str_replace(':', '%3A', str_replace(' ', '%20', date("Y-m-d G:i", strtotime((-1 * $minEjecucionProgramada) . ' minutes'))));
...
$urlProposals = "https://www.bks.com.mx/bcorp/api/index.php/proposals?limit=" . $limiteElementosApi . "&sqlfilters=te.acpd%3D1%20and%20t.fk_statut%3E0%20and%20t.entity%3D1%20and%20t.tms%3E'" . $dateSis . "'";
...
$urlContacts = "https://www.bks.com.mx/bcorp/api/index.php/contacts?limit=" . $limiteElementosApi . "&sqlfilters=te.acpd%3D1%20and%20t.statut%3D1%20and%20t.entity%3D1%20and%20te.mgpd%3D1%20and%20t.tms%3E'" . $dateSis . "'";
...
$urlThirdparties = "https://www.bks.com.mx/bcorp/api/index.php/thirdparties?limit=" . $limiteElementosApi . "&sqlfilters=te.acpd%3D1%20and%20t.status%3D1%20and%20t.entity%3D1%20and%20te.mgpd%3D1%20and%20(t.client%3D1%20or%20t.client%3D2%20or%20t.client%3D3)%20and%20t.tms%3E'" . $dateSis . "'";
...
////////////////////////////////////////////
//FUNCION API PIPEDRIVE POST PUT GET DELETE
    ...
    foreach ($listThirds as $posicion => $thirds) {
        try {
            if (isset($thirds["code_client"]) && !empty($thirds["code_client"]) && strlen($thirds["code_client"]) == $lengthCodeClient) {
                ...
                $resThirdsPipe = CallAPIPipe("PUT", $urlThirdpartiesEditPipe . $idThirdsPipe . $urlKeyPipe, $dataThirdsPipe);
                $resThirdsPipe = json_decode($resThirdsPipe, true);

                if (isset($resThirdsPipe["error"])) {
    ...
    foreach ($listContacts as $posicion => $contact) {
        try {
            if (isset($contact["socid"]) && !empty($contact["socid"])) {
                ...
                $resContactPipe = CallAPIPipe("PUT", $urlContactsEditPipe . $idContactPipe . $urlKeyPipe, $dataContactPipe);
                $resContactPipe = json_decode($resContactPipe, true);


                if (isset($resContactPipe["error"])) {
...
if (!isset($listProposals["error"]) && count($listProposals) > 0) {
    ...
    foreach ($listProposals as $posicion => $proposal) {
        try {
            ...
            $resProposalPipe = CallAPIPipe("PUT", $urlProposalsEditPipe . $idProposalPipe . $urlKeyPipe, $dataProposalPipe);
            ...
            if (isset($resProposalPipe["success"]) && $resProposalPipe["success"]) {
                ...
                //////////////////////////
                //OBTENER PRODUCTOS
                $listProposalsProductGet = CallAPIPipe("GET", $urlProposalsProductPipe . $idProposalsPipe . "/products" . $keyPipeDrive);
                $listProposalsProductGet = json_decode($listProposalsProductGet, true);

                foreach ($listProposalsProductGet["data"] as $posicionProduct => $proposalProductDelete) {
                    //////////////////////////
                    //ELIMINAR PRODUCTOS
                    $idProductDelete = $proposalProductDelete["id"];

                    $listProposalsProductDelete = CallAPIPipe("DELETE", $urlProposalsProductPipe . $idProposalsPipe . "/products/" . $idProductDelete . $keyPipeDrive);
                    $listProposalsProductDelete = json_decode($listProposalsProductDelete, true);
                }
...
echo "Proceso Actualizar Finalizado OK";
